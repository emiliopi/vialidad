...
export const Distritos = () => {
  ...
  // Carga de distritos paginados server-side
  const fetchDistritos = async () => {
    setLoading(true);
    try {
      const data = await distritoService.getDistritos(page, limit, search);
  ...
  const handleOpenEditModal = (distrito) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (editingDistrito) {
        // Modo Edición
        await distritoService.updateDistrito(editingDistrito.codigo_distrito, payload);
        toast.success('¡Distrito actualizado correctamente!');
      } else {
        // Modo Creación
        await distritoService.createDistrito(payload);
  ...
  // Eliminar distrito
  const handleDeleteDistrito = async (id, nombre) => {
    if (!window.confirm(`¿Estás seguro de que deseas eliminar permanentemente el distrito "${nombre}"?`)) {
      return;
    }

    setDeleteLoading(id);
    try {
      await distritoService.deleteDistrito(id);
      toast.success('Distrito eliminado exitosamente.');
      fetchDistritos();
    } catch (err) {
      console.error(err);
      const errMsg = err.response?.data?.detail || err.response?.data?.error || 'Error al intentar eliminar el distrito.';
      toast.error(errMsg);
    } finally {
  ...
  const hasAccess = currentUser?.menus?.some(m => m.path === '/distritos') || currentUser?.role === 'Super Admin' || currentUser?.role === 'Admin';
  if (!hasAccess) {
    return (
      <DashboardLayout>
      ...
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
              Administra los distritos y municipios utilizados para el cobro y registro de tasas de vialidades.
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
          <div className="relative w-full flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar distritos por nombre..."
              value={search}
              onChange={(e) => {
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center text-slate-500 dark:text-slate-400">
              ...
              <p className="text-sm font-semibold">No se encontraron distritos</p>
              <p className="text-xs text-slate-400 mt-1">Prueba a realizar otra búsqueda o registra un nuevo distrito.</p>
            </div>
          ) : (
              ...
              <table className="w-full border-collapse text-left text-sm text-slate-700 dark:text-slate-300">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                    ...
                    <th className="px-6 py-4">Nombre del Distrito</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {distritos.map((d) => (
                    <tr key={d.codigo_distrito} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/10 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <Badge variant={d.activo ? 'success' : 'danger'}>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="sm" onClick={() => handleOpenEditModal(d)} className="p-1">
                          ...
                          <Button
                            variant="danger"
                            size="sm"
                            disabled={deleteLoading === d.codigo_distrito}
                            loading={deleteLoading === d.codigo_distrito}
                            onClick={() => handleDeleteDistrito(d.codigo_distrito, d.nombre)}
                            className="p-1"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-4 h-4">
          ...
          {totalPages > 1 && (
            <div className="border-t border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/20">
              <span className="text-xs text-slate-500 dark:text-slate-400">
                Mostrando página {page} de {totalPages} ({total} distritos en total)
              </span>
        ...
        {/* Modal de Crear / Editar Distrito */}
        <Modal
          isOpen={isModalOpen}
          ...
          footer={
            <div className="flex justify-end gap-2.5 w-full">
              ...
              <Button variant="primary" onClick={handleSubmit(onSubmit)} loading={submitting} size="sm">
                {editingDistrito ? 'Actualizar Distrito' : 'Registrar Distrito'}
              </Button>
            </div>
          }
        >
